...
class Materi extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $config['base_url']     = site_url('materi/index');                       //arah site dan function nya
    $config['total_rows']   = $this->db->count_all('tb_doc_sambutan');          //total row
    $config['per_page']     = 8;                                            //show data per halaman
    $choice                 = $config["total_rows"] / $config["per_page"];
    $config["num_links"]    = floor($coba);

    if ($keyword == null) {
      $data['doc_sambutan']  = $this->M_sambutan->tampil($config['per_page'], $data['page']);
    } else {
      $data['doc_sambutan']  = $this->M_sambutan->search($keyword);
    }
  ...
  {
    $tb_doc_sambutan = $this->M_sambutan->getDetail('tb_doc_sambutan', $id);
